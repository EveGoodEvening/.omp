...
Step 1. **Gather context** — ${gatherInstruction} Use max thinking/reasoning effort for research subagents.
Step 2. **Plan durably** — Delegate to a dedicated (max thinking/reasoning effort) plan subagent to create or update the durable plan and checklist/progress tracker so future sessions can resume the work. The orchestrator coordinates and validates the planning artifact only; it must not design the plan itself. Skip this step for an in-progress resume unless the tracker is missing, stale, or insufficient to identify the current/next implementation item.
